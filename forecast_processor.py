...
def load_po_data(file_path: Path) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        logger.error(f"Could not decode JSON from {file_path}. Is it valid JSON? Cannot perform forecasting.")
...
def clean_forecast_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Inflow (USD)" in df.columns:
        ...
        df["Inflow (USD)"] = df["Inflow (USD)"].round(2)
        df["Inflow (USD)"] = df["Inflow (USD)"].fillna(0.0)
...
def save_forecast_csv(df: pd.DataFrame, output_csv_path: str) -> None:
...
def generate_and_save_pivot(df: pd.DataFrame, pivot_excel_path: str) -> None:
...
def run_forecast_processing(
    input_json_path: str = "./output/purchase_orders.json",
    ...
    po_data_list = load_po_data(file_path)
